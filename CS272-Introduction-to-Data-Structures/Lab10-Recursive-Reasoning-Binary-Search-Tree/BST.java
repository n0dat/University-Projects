...
public class BST {
	...
	// inserts node in correct spot in tree
	...
	// searches a tree for input value recursively
	...
	// searches a tree for input value non-recursively
	public BSTNode searchNonRecursion(int e) {
		
		// I could not get this to work properly without
		// destroying the tree
		/*
		BSTNode head = root;
		while (head.getData() != e && !head.isLeaf()) {	
			if (e > head.getData())
				head = getRightSubtree().root;
			if (e < head.getData())
				head = getLeftSubtree().root;
	...
	// returns the kth largset node value in tree
	// this works, but the return, returns the root, not the node that was most recently checked
	public static BSTNode kthLargest(BST main, int k) {
		if (k == 1) {
			System.out.println("  " + main.root.getData());
			return main.root;
		}
		System.out.println(main.root.getData());
		System.out.println(k);
		kthLargest(main.getRightSubtree(), k - 1);
		return main.root;
	}
	...
	private static void test1() throws Exception{
		...
		BSTNode temp = kthLargest(tree, 2);
